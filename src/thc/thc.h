...
// Simple definition to aid platform portability (only remains of former
// Portability.h)
int strcmp_ignore(
    const char *s, const char *t); // return 0 if case-insensitive match

// Fast test for is square white or black. Intend to move this to namespace thc
// when convenient...
inline bool is_dark(int sq)
{
    bool dark =
        (!(sq & 8) &&
         (sq & 1)) // eg (a8,b8,c8...h8) && (b8|d8|f8|h8) odd rank + odd file
        || ((sq & 8) &&
            !(sq &
              1)); // eg (a7,b7,c7...h7) && (a7|c7|e7|g7) even rank + even file
    return dark;
}
...
#endif // MOVE_H
/****************************************************************************
 * ChessPositionRaw.h Chess classes - A raw chess position, could be used as a C
 *style POD Author:  Bill Forster License: MIT license. Full text of license is
 *in associated file LICENSE Copyright 2010-2020, Bill Forster <billforsternz at
 *gmail dot com>
 ****************************************************************************/
...
class ChessPosition : public ChessPositionRaw
{
  public:
    ...
    {
        return (
            white == other.white && 0 == memcmp(&squares, &other.squares, 64) &&
    ...
    bool operator<(const ChessPosition &other) const
    {
        ...
        if (!temp)
            return (
                (int)groomed_enpassant_target() <
                (int)other.groomed_enpassant_target());
    ...
    //  SQUARE_INVALID unless there is an opposition pawn in position to make
    //  the capture
    ...
    // Calculate a hash value for position (not same as CompressPosition
    // algorithm hash)
...
class ChessRules : public ChessPosition
{
  public:
    ...
    void
    Init() // TODO == ChessRules::Init() should call ChessPosition::Init() right
           // ????!!!! Thoughts: Maybe - but can't do this casually. For example
           // we would need to change the code that converts ChessPosition to
           // ChessRules below, both the copy constructor and assignment
           // operator use ChessRules::Init() at a time when it would be
           // disastrous to set the initial position (because we have carefully
           // copied a position into the ChessRules object)
    {
        history_idx = 1; // prevent bogus repetition draws
        history[0].src =
            a8; // (look backwards through history stops when src==dst)
        history[0].dst = a8;
        detail_idx     = 0;
    }
    ...
    //  Test for legal position, sets reason to a mask of possibly multiple
    //  reasons
    ...
    // Transform a position with W to move into an equivalent with B to move and
    // vice-versa
    void Transform();
  ...
  protected:
    ...
    void
    ShortMoves(MOVELIST *l, Square square, const lte *ptr, SPECIAL special);
...
#endif // CHESSRULES_H
/****************************************************************************
 * ChessEvaluation.h Chess classes - Simple chess AI, leaf scoring function for
 *position Author:  Bill Forster License: MIT license. Full text of license is
 *in associated file LICENSE Copyright 2010-2020, Bill Forster <billforsternz at
 *gmail dot com>
 ****************************************************************************/
